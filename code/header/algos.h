...
vector<Point> get_points_from_rails(Mat& src, Mat& rails_masks, Size& size) // на данный момент не используется - не обращайте внимание 
{
...
vector<Mat> get_3d_coords(Camera& camera, vector<Point> pts2d) // на данный момент не используется - не обращайте внимание 
{
	...
	float Zcu = d * (fx * r11 + cx * r31) / (pts2d[1].x - pts2d[0].x); // äëÿ âåðõíåé ïàðû 
	float Zcl = d * (fx * r11 + cx * r31) / (pts2d[3].x - pts2d[2].x); // äëÿ íèæíåé ïàðû
	vector<Vec3f> pts;
	for (auto& pt2d : pts2d)
	{
