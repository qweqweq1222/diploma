...
struct SnavelyReprojectionErrorWorld { // определяем нашу задачу квадратичной минимизации  
	...
	bool operator()(const T* const alpha_t, const T* const pt3d, T* residuals) const {
		...
		/* пробовался и регуляризатор на то, чтобы не выходили за пределы экрана (можно добавить, но нужно передать по мимо fx,fy... еще ширину и высоту экрана 
		// и изменить {2,6,3} в конструкторе на {3,6,3} - первое число - размерность нашего вектора 
		residuals[2] = predicted_x > T(0) && predicted_x < T(width) ? T(0) : T(1000);
		residuals[3] = predicted_y > T(0) && predicted_y < T(height) ? T(0) : T(1000);
		*/
		return true;
	...
	static ceres::CostFunction* Create(const double observed_x,
		...
		return (new ceres::AutoDiffCostFunction<SnavelyReprojectionErrorWorld, 2, 6, 3>(
